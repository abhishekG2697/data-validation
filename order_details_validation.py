...
class OrderDetailsValidator:
    ...
    def generate_report(self, output_file: str = None):
        ...
        if self.validation_results['missing_in_iceberg']:
            report.append(f"\n⚠️  LINE ITEMS MISSING IN ICEBERG ({len(self.validation_results['missing_in_iceberg'])})")
            report.append("-" * 40)
        ...
        if self.validation_results['missing_in_secor']:
            report.append(f"\n⚠️  LINE ITEMS MISSING IN SECOR ({len(self.validation_results['missing_in_secor'])})")
            report.append("-" * 40)
